            ...
            <div className="w-screen h-screen bg-gray-100 dark:bg-gray-800 flex items-center justify-center transition-colors">
                <div className="bg-white shadow-md rounded-md p-5 dark:bg-gray-900 transition-colors">
                    ...
                    <div className="grid grid-cols-1 gap-2 pt-10 w-64">
                        ...
                        <div className="flex items-center justify-center cursor-pointer py-4 dark:text-white" onClick={ () => addToast('Esqueci minha senha') }>
                            Esqueci minha senha 🥴
                        </div>
                        <button className="bg-purple-400 h-10 w-full rounded-sm text-white font-bold" onClick={ () => login({ email: email, password: pass }) }>{ isLoading ? '🤚' : 'Login' }</button>
                    </div>
                </div>
            </div>
        </div>
